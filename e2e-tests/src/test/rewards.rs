...
use crate::{
    ...
    test::utility::enable_validator,
    Config,
};
use aleph_client::{
    change_validators, get_block_hash, get_current_session, get_era_reward_points_result,
    get_exposure, get_session, get_session_period, get_sessions_per_era, wait_for_at_least_session,
    wait_for_finalized_block, wait_for_next_era, AnyConnection, KeyPair, RewardPoint,
...
pub fn test_disable_node(config: &Config) -> anyhow::Result<()> {
    ...
    let next_era_block = era * session_period * sessions_per_era;
    let next_era_block_hash = get_block_hash(&controller_connection, next_era_block);
    let end_session = get_session(&controller_connection, Some(next_era_block_hash));
    let next_era_block = next_era_block + sessions_per_era * session_period;
    wait_for_finalized_block(&root_connection, next_era_block)?;
    let era_end_session = end_session + sessions_per_era;
...
pub fn test_points_and_payouts(config: &Config) -> anyhow::Result<()> {
    const MAX_DIFFERENCE: f64 = 0.05;
