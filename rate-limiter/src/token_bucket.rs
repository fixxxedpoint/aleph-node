...
mod tests {
    ...
    fn token_bucket_sanity_check() {
        ...
        assert_eq!(
            rate_limiter
                .rate_limit(9)
                .and_then(|cont| cont(now + Duration::from_secs(1))),
            None
        );
        ...
        assert_eq!(
            rate_limiter
                .rate_limit(8)
                .and_then(|cont| cont(now + Duration::from_secs(3))),
            None
        );
    }
    ...
    fn no_slowdown_while_within_rate_limit() {
        ...
        assert_eq!(
            rate_limiter
                .rate_limit(5)
                .and_then(|cont| cont(now + Duration::from_secs(2))),
            None
        );
        assert_eq!(
            rate_limiter
    ...
    fn slowdown_when_limit_reached() {
        ...
        assert_eq!(rate_limiter.rate_limit(10).and_then(|cont| cont(now)), None);

        // we should w))ait some time after reaching the limit
        assert!(rate_limiter
            .rate_limit(1)
            .and_then(|cont| cont(now))
            .is_some());

        assert_eq!(
            rate_limiter.rate_limit(19).and_then(|cont| cont(now)),
    ...
    fn buildup_tokens_but_no_more_than_limit() {
        ...
        assert_eq!(
            rate_limiter
                .rate_limit(40)
                .and_then(|cont| cont(now + Duration::from_secs(10))),
            Some(Duration::from_secs(3)),
        );
        assert_eq!(
            rate_limiter
                .rate_limit(40)
                .and_then(|cont| cont(now + Duration::from_secs(11))),
